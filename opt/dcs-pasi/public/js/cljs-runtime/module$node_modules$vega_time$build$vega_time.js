...
dayofyear:d=>localDayOfYear(d),week:d=>localWeekNum(d),weekday:(d,y)=>weekday(localWeekNum(d),d.getDay(),localYear(y).getDay()),day:(d,y)=>weekday(1,d.getDay(),localYear(y).getDay())},localInv={quarter:q=>3*q,week:(w,y)=>weekday(w,0,localYear(y).getDay())},utcGet={year:d=>d.getUTCFullYear(),quarter:d=>Math.floor(d.getUTCMonth()/3),month:d=>d.getUTCMonth(),date:d=>d.getUTCDate(),hours:d=>d.getUTCHours(),minutes:d=>d.getUTCMinutes(),seconds:d=>d.getUTCSeconds(),milliseconds:d=>d.getUTCMilliseconds(),
dayofyear:d=>utcDayOfYear(d),week:d=>utcWeekNum(d),day:(d,y)=>weekday(1,d.getUTCDay(),utcFirst(y)),weekday:(d,y)=>weekday(utcWeekNum(d),d.getUTCDay(),utcFirst(y))},utcInv={quarter:q=>3*q,week:(w,y)=>weekday(w,0,utcFirst(y))},timeIntervals={year:d3Time.timeYear,quarter:d3Time.timeMonth.every(3),month:d3Time.timeMonth,week:d3Time.timeWeek,date:d3Time.timeDay,day:d3Time.timeDay,dayofyear:d3Time.timeDay,hours:d3Time.timeHour,minutes:d3Time.timeMinute,seconds:d3Time.timeSecond,milliseconds:d3Time.timeMillisecond},
utcIntervals={year:d3Time.utcYear,quarter:d3Time.utcMonth.every(3),month:d3Time.utcMonth,week:d3Time.utcWeek,date:d3Time.utcDay,day:d3Time.utcDay,dayofyear:d3Time.utcDay,hours:d3Time.utcHour,minutes:d3Time.utcMinute,seconds:d3Time.utcSecond,milliseconds:d3Time.utcMillisecond},Milli="year month date hours minutes seconds milliseconds".split(" "),Seconds=Milli.slice(0,-1),Minutes=Seconds.slice(0,-1),Hours=Minutes.slice(0,-1),Day=Hours.slice(0,-1),Month=["year","month"],Year=["year"],intervals=[[Seconds,
1,1E3],[Seconds,5,5E3],[Seconds,15,15E3],[Seconds,30,3E4],[Minutes,1,6E4],[Minutes,5,3E5],[Minutes,15,9E5],[Minutes,30,18E5],[Hours,1,36E5],[Hours,3,108E5],[Hours,6,216E5],[Hours,12,432E5],[Day,1,864E5],[["year","week"],1,6048E5],[Month,1,2592E6],[Month,3,7776E6],[Year,1,31536E6]];exports.DATE="date";exports.DAY="day";exports.DAYOFYEAR="dayofyear";exports.HOURS="hours";exports.MILLISECONDS="milliseconds";exports.MINUTES="minutes";exports.MONTH="month";exports.QUARTER="quarter";exports.SECONDS="seconds";
exports.TIME_UNITS=TIME_UNITS;exports.WEEK="week";exports.YEAR="year";exports.dayofyear=function(d){return localDayOfYear(new Date(d))};exports.timeBin=function(opt){var ext=opt.extent;opt=opt.maxbins||40;var target=Math.abs(vegaUtil.span(ext))/opt;let i$jscomp$0=d3Array.bisector(i=>i[2]).right(intervals,target);i$jscomp$0===intervals.length?(target=Year,ext=d3Array.tickStep(ext[0]/31536E6,ext[1]/31536E6,opt)):i$jscomp$0?(i$jscomp$0=intervals[target/intervals[i$jscomp$0-1][2]<intervals[i$jscomp$0][2]/
target?i$jscomp$0-1:i$jscomp$0],target=i$jscomp$0[0],ext=i$jscomp$0[1]):(target=Milli,ext=Math.max(d3Array.tickStep(ext[0],ext[1],opt),1));return{units:target,step:ext}};exports.timeFloor=function(units,step){return floor(units,step||1,localGet,localInv,localDate)};exports.timeInterval=function(unit){return timeIntervals[unit]};exports.timeOffset=function(unit,date,step){return(unit=timeIntervals[unit])?unit.offset(date,step):void 0};exports.timeSequence=function(unit,start,stop,step){return(unit=
timeIntervals[unit])?unit.range(start,stop,step):void 0};exports.timeUnitSpecifier=function(units,specifiers){specifiers=vegaUtil.extend({},defaultSpecifiers,specifiers);units=timeUnits(units);const n=units.length;let fmt="",start,end,key;for(start=0;start<n;)for(end=units.length;end>start;--end)if(key=units.slice(start,end).join("-"),null!=specifiers[key]){fmt+=specifiers[key];start=end;break}return fmt.trim()};exports.timeUnits=timeUnits;exports.utcFloor=function(units,step){return floor(units,
step||1,utcGet,utcInv,utcDate)};exports.utcInterval=function(unit){return utcIntervals[unit]};exports.utcOffset=function(unit,date,step){return(unit=utcIntervals[unit])?unit.offset(date,step):void 0};exports.utcSequence=function(unit,start,stop,step){return(unit=utcIntervals[unit])?unit.range(start,stop,step):void 0};exports.utcdayofyear=function(d){return utcDayOfYear(new Date(d))};exports.utcweek=function(d){return utcWeekNum(new Date(d))};exports.week=function(d){return localWeekNum(new Date(d))};
