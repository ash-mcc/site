...
{{var config=model.config,_legend$encode3,$jscomp$inline_1778=l.combine();l=Object.assign({},$jscomp$inline_1778);const disable=$jscomp$inline_1778.disable;$jscomp$inline_1778=$jscomp$inline_1778.labelExpr;l=(delete l.disable,delete l.labelExpr,delete l.selections,l);if(disable)var JSCompiler_inline_result=void 0;else{!1===config.aria&&void 0==l.aria&&(l.aria=!1);if(null!==(_legend$encode3=l.encode)&&void 0!==_legend$encode3&&_legend$encode3.symbols){config=l.encode.symbols.update;!config.fill||"transparent"===
config.fill.value||config.stroke||l.stroke||(config.stroke={value:"transparent"});for(var property of LEGEND_SCALE_CHANNELS)l[property]&&delete config[property]}l.title||delete l.title;if(void 0!==$jscomp$inline_1778){var _legend$encode4,_legend$encode4$label;property=$jscomp$inline_1778;null!==(_legend$encode4=l.encode)&&void 0!==_legend$encode4&&null!==(_legend$encode4$label=_legend$encode4.labels)&&void 0!==_legend$encode4$label&&_legend$encode4$label.update&&isSignalRef(l.encode.labels.update.text)&&
(property=replaceAll($jscomp$inline_1778,"datum.label",l.encode.labels.update.text.signal));_legend$encode4=l;_legend$encode4$label={signal:property};var _legend$encode2,_legend$encode2$part,_legend$encode$part,_legend$encode$part$u;null!==(JSCompiler_inline_result=_legend$encode4.encode)&&void 0!==JSCompiler_inline_result?JSCompiler_inline_result:_legend$encode4.encode={};null!==(_legend$encode2$part=(_legend$encode2=_legend$encode4.encode).labels)&&void 0!==_legend$encode2$part?_legend$encode2$part:
...
DataSourceType.Main,outputNodeRefCounts);head=outputNodes[_StackNode$makeFromEn]=_ImputeNode$makeFromE;if(isUnitModel(model))for(var $jscomp$inline_1936 of entries$1(null!==(_model$component$sele=model.component.selection)&&void 0!==_model$component$sele?_model$component$sele:{})){const [selection,selCmpt]=$jscomp$inline_1936;var _model$component$sele;_model$component$sele=model.getName("lookup_".concat(selection));model.component.data.outputNodes[_model$component$sele]=selCmpt.materialized=new OutputNode(new FilterNode(_ImputeNode$makeFromE,
model,{param:selection}),_model$component$sele,DataSourceType.Lookup,model.component.data.outputNodeRefCounts)}_model$component$sele=null;if(isFacetModel(model)){var _makeJoinAggregateFro;$jscomp$inline_1936=model.getName("facet");{_model$component$sele=head;const {row,column}=model.facet;if(row&&column){_StackNode$makeFromEn=null;for(JSCompiler_inline_result of[row,column])if(isSortField(JSCompiler_inline_result.sort)){const {field,op="min"}=JSCompiler_inline_result.sort;_model$component$sele=_StackNode$makeFromEn=
new JoinAggregateTransformNode(_model$component$sele,{joinaggregate:[{op,field,as:facetSortFieldName(JSCompiler_inline_result,JSCompiler_inline_result.sort,{forAs:!0})}],groupby:[vgField(JSCompiler_inline_result)]})}var JSCompiler_inline_result=_StackNode$makeFromEn}else JSCompiler_inline_result=null}head=null!==(_makeJoinAggregateFro=JSCompiler_inline_result)&&void 0!==_makeJoinAggregateFro?_makeJoinAggregateFro:head;_model$component$sele=new FacetNode(head,model,$jscomp$inline_1936,_ImputeNode$makeFromE.getSource());
outputNodes[$jscomp$inline_1936]=_model$component$sele}return Object.assign({},model.component.data,{outputNodes,outputNodeRefCounts,raw:_ParseNode$makeWithAn2,main:_ImputeNode$makeFromE,facetRoot:_model$component$sele,ancestorParse})}function parseUnitAxes(model){return POSITION_SCALE_CHANNELS.reduce((axis,channel)=>{model.component.scales[channel]&&(axis[channel]=[parseAxis(channel,model)]);return axis},{})}function mergeAxisComponents(mergedAxisCmpts,childAxisCmpts){if(mergedAxisCmpts){if(mergedAxisCmpts.length!==
